...
def landmarksDetection(image, results, draw=False):
    ...
    mesh_coordinates = [(int(point.x * image_width), int(point.y * image_height)) for point in results.landmark]
    if draw :
...
# To improve performance, optionally mark the image as not writeable to
        ...
        if results.face_landmarks:
            mesh_coordinatess = landmarksDetection(frame, results.face_landmarks, True)
